...
从指定指定地址开始写入指定长度的数据
特别注意：
			因为STM32F4的扇区太大，没办法本地保存扇区数据，所以本函数写地址如果非0XFF，
			那么会先擦除整个扇区并且不保存扇区数据。所以写非0XFF的地址，将导致整个扇区
			数据丢失。建议写之前确保没有扇区里没有重要数据，最好是整个扇区先擦除了，再
			慢慢往后写。
该函数对OTP区也有效，可以用来写OTP区
OTP区对应的地址是：0X1FFF7800~0X1FFF7A0F(注意:最后16个字节用于OTP数据所块锁定，不能乱写)
WriteAddr:起始地址，此地址必须为4的倍数
pBuffer:数据指针
NumToWrite:字数（32位），就是要写入32位数据的个数
***************************************************************************************/
void STMFLASH_Write(u32 WriteAddr, u32 * pBuffer, u32 NumToWrite)
{
	...
	if(WriteAddr<STM32_FLASH_BASE||WriteAddr%4)					//非法地址
		return	;
	HAL_FLASH_Unlock();											//解锁
	addrx=WriteAddr;											//写入的开始地址
	endaddr=WriteAddr+NumToWrite*4;								//写入的结束地址

	if(addrx<0x1FFF0000)
		{
			while(addrx<endaddr)							//扫清一切障碍（对非0XFFFFFFFF）的地方呆擦除这个扇区
				{
					if(STMFLASH_ReadWord(addrx)!=0xFFFFFFFF)			//有非0XFFFFFFFF的地方，要擦除这个扇区
						{
							...
							if(HAL_FLASHEx_Erase(&FlashEraseInit,&SectorError)!=HAL_OK)	//发生错误
								break;
						}
					else
						addrx+=4;
