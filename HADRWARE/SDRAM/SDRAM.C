...
void SDRAM_Init(void)
{
	...
	SDRAM_Handler.Init.SDBank=FMC_SDRAM_BANK1;							//第一个SDRAM BANK(片选信号FMC_SDNE0决定)
	...
	SDRAM_Handler.Init.MemoryDataWidth=FMC_SDRAM_MEM_BUS_WIDTH_16;		//数据宽度
	...
	SDRAM_Handler.Init.WriteProtection=FMC_SDRAM_WRITE_PROTECTION_DISABLE;		//禁用写保护（可以正常写指令）
	SDRAM_Handler.Init.SDClockPeriod=FMC_SDRAM_CLOCK_PERIOD_2;					//SDRAM时钟为HCLK/2=180M/2=90M=11.1ns
	SDRAM_Handler.Init.ReadBurst=FMC_SDRAM_RBURST_ENABLE;						//使能突发模式
	...
	SDRAM_Timing.RPDelay=2;									//行预充电延迟周期为2个时钟周期
	...
	SDRAM_Initialization_Sequence(&SDRAM_Handler);			//发送SDRAM初始化序列
		...
		所使用的SDRAM刷新周期为64ms,SDCLK=180/2=90Mhz,行数为8192（2^13）
		所以COUNT=64*1000*90/8192-20=683
		*******************************************************/
...
void SDRAM_Initialization_Sequence(SDRAM_HandleTypeDef * hsdram)
{
	...
	BIT10-BIT11保留
	***************************************************************************************/
	temp=(u32)SDRAM_MODEREG_BURST_LENGTH_1				|		//设置突发长度：1（可以是：1/2/4/8）
			  SDRAM_MODEREG_BURST_TYPE_SEQUENTIAL		|		//设置突发类型：连续（可以是连续/交错）
			  SDRAM_MODEREG_CAS_LATENCY_3				|		//设置CAS值：3（可以是2/3）
			  SDRAM_MODEREG_OPERATING_MODE_STANDARD		|		//设置操作模式0，标准模式
			  SDRAM_MODEREG_WRITEBURST_MODE_SINGLE;				//设置突发写模式1，单点访问

	SDRAM_Send_Cmd(0,FMC_SDRAM_CMD_LOAD_MODE,1,temp);			//设置SDRAM模式寄存器
}
/********************************************************************************
SDRAM底层驱动，引脚配置，时钟使能
...
void HAL_SDRAM_MspInit(SDRAM_HandleTypeDef * hsdram)
{
	...
	HAL_GPIO_Init(GPIOC,&GPIO_Initure);						//初始化PC0,PC2,PC3

	GPIO_Initure.Pin=GPIO_PIN_0|GPIO_PIN_1|GPIO_PIN_8|GPIO_PIN_9|	
					 GPIO_PIN_10|GPIO_PIN_14|GPIO_PIN_15;					
	HAL_GPIO_Init(GPIOD,&GPIO_Initure);						//初始化PD0,PD1,PD8,PD9,PD10,PD14,PD15
...
/*********************************************************************************************
向SDRAM发送命令
Bankx: 	0，向Bank5上的SDRAM发送指令
		1，向Bank6上的SDRAM发送指令
CMD:指令  （0，正常模式			1，时钟配置使能			2，预充电所有存储区				3，自动刷新
		  4，加载模式寄存器				5，自刷新			6，掉电）
refresh:自刷新次数
regval:模式寄存器的定义
返回值：0，正常
		1，失败
